...
# --- DICTIONNAIRE DES 19 CATÉGORIES (FIDÈLE À LA CAPTURE D'ÉCRAN) ---
CATEGORIES = {
    "1. SMARTPHONE": [
        "smartphone",
        "téléphone",
...
def build_regex_pattern(keyword: str) -> str:
    ...
    if len(words) > 1:
        # Expression composée de plusieurs mots (ex: "jeux vidéo")
        first_part = r"\s+".join(re.escape(w) for w in words[:-1])
        last_part = re.escape(words[-1]) + r"(?:s|x)?\b"
        return r"\b" + first_part + r"\s+" + last_part
    else:
        # Mot unique avec bordure de mot stricte au début et tolérance s/x à la fin
...
def generate_excel_from_csv():
    ...
    with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl") as writer:
        ...
        for cat_name in CATEGORIES.keys():
            sheet_title = cat_name[:31]  # Contrainte de longueur Excel (31 caractères max)
